...
void main() {
  group('Game State Data Conversion for Multiplayer Persistence', () {
    ...
    test(
      'should enable bidirectional game state conversion for multiplayer persistence and synchronization',
      () {
        ...
        expect(
          domainGameState.roomId,
          'competitive-room-456',
          reason: 'Room context should be preserved for multiplayer continuity',
        );
        expect(
          domainGameState.status,
          GameStatus.playing,
          reason: 'Game status should enable proper state management',
        );
        expect(
          domainGameState.players.length,
          2,
          reason: 'All competitive players should be restored from persistence',
        );
        expect(
          domainGameState.players.first.name,
          'Strategic Player',
          reason: 'Player identity should be maintained across sessions',
        );

        // Domain to database conversion for state synchronization
        final synchronizedData = converter.toJson(testGameState);
        expect(
          synchronizedData['room_id'],
          'competitive-room-456',
          reason:
              'Room identification should be preserved for multiplayer coordination',
        );
        expect(
          synchronizedData['status'],
          'playing',
          reason: 'Game status should be consistently represented',
        );
        expect(
          synchronizedData['game_data']['players'],
          hasLength(2),
          reason: 'Complete player roster should be synchronized',
        );

        // Roundtrip conversion ensures data integrity for multiplayer persistence
        final roundtripState = converter.fromJson(
          converter.toJson(testGameState),
        );
        expect(
          roundtripState.roomId,
          testGameState.roomId,
          reason: 'Room context should survive roundtrip conversion',
        );
        expect(
          roundtripState.players.length,
          testGameState.players.length,
          reason:
              'Player roster should remain complete through persistence cycles',
        );
        expect(
          roundtripState.status,
          testGameState.status,
          reason:
              'Game state should maintain consistency for competitive integrity',
        );
      },
    );
    ...
    test('should handle edge cases for robust multiplayer game persistence', () {
      ...
      expect(
        newGameState.roomId,
        'new-competitive-room',
        reason: 'New game rooms should be properly initialized',
      );
      expect(
        newGameState.status,
        GameStatus.waitingToStart,
        reason: 'Pre-game status should be correctly handled',
      );
      expect(
        newGameState.players,
        isEmpty,
        reason: 'Empty player roster should be valid for game initialization',
      );
      ...
      expect(
        tempGameState.roomId,
        'temp-competitive-room',
        reason: 'Temporary games should maintain room identity',
      );
      ...
      expect(
        legacyGameState.roomId,
        'mixed-case-room',
        reason: 'Legacy camelCase data should be compatible',
      );
      expect(
        legacyGameState.players.length,
        1,
        reason: 'Legacy player data should be preserved',
      );
    });
  });
}

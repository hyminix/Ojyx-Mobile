...
void main() {
  group('Room Repository Behavior for Multiplayer Game Coordination', () {
    ...
    test(
      'should enable competitive room creation and participant management for strategic multiplayer gaming',
      () async {
        ...
        expect(
          ...
          strategicHostId,
          reason:
              'Room should establish proper host authority for competitive control',
        );
        expect(
          createdRoom.maxPlayers,
          competitiveCapacity,
          reason:
              'Capacity should support strategic group competitive gameplay',
        );
        expect(
          createdRoom.status,
          RoomStatus.waiting,
        ...
        expect(
          ...
          [strategicHostId],
          reason: 'Host should be initial participant in competitive room',
        );
        ...
        expect(
          ...
          throwsException,
          reason:
              'Repository should propagate creation failures for proper error handling',
        );
      },
    );

    test(
      'should coordinate competitive participant joining for balanced multiplayer room composition',
      () async {
        ...
        expect(
          joinedRoom!.playerIds.length,
          2,
          reason: 'Room should expand to include new competitive participant',
        );
        expect(
          joinedRoom.playerIds.contains(newCompetitorId),
          true,
          reason: 'New competitor should be properly registered in room roster',
        );
        expect(
          joinedRoom.status,
          RoomStatus.waiting,
          reason:
              'Room should remain open for additional competitive participants',
        );
        ...
        expect(
          () => repository.joinRoom(
            roomId: 'full-room',
            playerId: 'rejected-player',
          ),
          throwsException,
          reason:
              'Repository should handle capacity limits and joining conflicts properly',
        );
      },
    );

    test(
      'should provide real-time room monitoring for synchronized competitive multiplayer coordination',
      () async {
        ...
        when(
          () => mockDatasource.watchRoom(liveCompetitiveRoomId),
        ).thenAnswer((_) => Stream.value(monitoredRoom));

        final roomMonitoringStream = repository.watchRoom(
          liveCompetitiveRoomId,
        );
        ...
        await expectLater(
          ...
          emits(monitoredRoom),
          reason:
              'Repository should provide real-time room updates for competitive coordination',
        );
      },
    );

    test(
      'should track competitive room events for real-time multiplayer participant coordination',
      () async {
        ...
        when(
          () => mockDatasource.watchRoomEvents(eventMonitoredRoomId),
        ).thenAnswer((_) => Stream.value(competitiveJoinEvent));

        final roomEventStream = repository.watchRoomEvents(
          eventMonitoredRoomId,
        );
        ...
        await expectLater(
          ...
          reason:
              'Repository should provide real-time competitive event tracking for participant coordination',
        );
      },
    );

    test(
      'should coordinate competitive game initialization and room state transition for organized multiplayer matches',
      () async {
        ...
        when(
          () => mockDatasource.getRoom(competitiveRoomId),
    ...
    test(
      'should discover competitive rooms for strategic multiplayer matchmaking opportunities',
      () async {
        ...
        expect(
          ...
          2,
          reason:
              'Repository should discover multiple competitive room options for strategic matchmaking',
        );
        expect(
          discoveredRooms[0].id,
          'strategic-arena-alpha-456',
          reason: 'First competitive room should be properly accessible',
        );
        expect(
          discoveredRooms[1].id,
          'tactical-arena-beta-789',
          reason:
              'Second competitive room should offer alternative strategic gameplay',
        );
        expect(

...
void main() {
  group('Room Participation for Competitive Multiplayer Gaming', () {
    ...
    test('should successfully enable competitive multiplayer participation when strategic rooms are available for joining', () async {
      ...
      when(() => mockRepository.getRoom(competitiveRoomId))
      ...
      expect(participatedRoom, isNotNull, reason: 'Join operation should return valid room');
      expect(participatedRoom!.playerIds, contains(strategicCompetitorId), reason: 'Strategic competitors should be successfully added to competitive rooms');
      expect(participatedRoom.playerIds.length, 2, reason: 'Room expansion should accommodate new competitive participants');
      expect(participatedRoom.status, RoomStatus.waiting, reason: 'Joined rooms should maintain availability for additional competitive participants');
    ...
    group('should enforce competitive gaming constraints to maintain fair and strategic multiplayer experiences', () {
      test('when attempting to join non-existent competitive rooms', () async {
        ...
        when(() => mockRepository.getRoom(nonExistentRoomId))
        ...
        expect(
          ...
          throwsException,
          reason: 'Non-existent rooms should be rejected to prevent invalid competitive participation',
        );
      });

      test('when attempting to join active competitive matches in progress', () async {
        ...
        when(() => mockRepository.getRoom(activeRoomId))
        ...
        expect(
          ...
          throwsException,
          reason: 'Active matches should reject new participants to maintain competitive fairness',
        );
      });

      test('when attempting to join capacity-reached competitive rooms', () async {
        ...
        when(() => mockRepository.getRoom(capacityReachedRoomId))
        ...
        expect(
          ...
          throwsException,
          reason: 'Capacity-reached rooms should reject additional participants to maintain strategic competitive balance',
        );
      });
    });
  });

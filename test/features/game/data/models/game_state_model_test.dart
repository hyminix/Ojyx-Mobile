...
void main() {
  group('GameStateModel', () {
    ...
    test(
      'should enable seamless game state persistence across clean architecture boundaries',
      () {
                ...
                GamePlayer(
                  ...
                  grid: const PlayerGrid(
                    cards: [
                      ...
                      [
                        Card(value: 8, isRevealed: false),
                        Card(value: 3, isRevealed: true),
                        Card(value: 0, isRevealed: false),
                  ...
                  grid: PlayerGrid(
                    cards: List.generate(
                      ...
                      (r) => List.generate(
                        4,
                        (c) =>
                            Card(value: r * 4 + c, isRevealed: r == 0 && c < 2),
                      ),
                    ),
                  ),
                  actionCards: [],
                ...
                GamePlayer(
                  ...
                  grid: PlayerGrid(
                    cards: List.generate(
                      ...
                      (r) => List.generate(
                        4,
                        (c) => Card(value: c <= 1 ? 0 : c, isRevealed: true),
                      ),
                    ),
                  ),
                  actionCards: [],
                ...
                GamePlayer(
                  ...
                  grid: PlayerGrid(
                    cards: List.generate(
                      ...
                      (r) => List.generate(
                        4,
                        (c) => Card(value: r + c + 3, isRevealed: true),
                      ),
                    ),
                  ),
                  actionCards: [],
        ...
        for (final scenario in persistenceScenarios) {
          ...
          if (scenario.expectations['preserves_initial_state'] == true) {
            expect(
              reconstructed.status,
              equals(GameStatus.waitingToStart),
              reason: '${scenario.name}: Initial state should be waiting',
            );
            expect(
              reconstructed.deck.length,
              equals(52),
            ...
            expect(
              ...
              equals(20),
              reason:
                  '${scenario.name}: Initial action deck should be complete',
            );
          }
          ...
          if (scenario.expectations['maintains_drawn_card'] == true) {
            ...
            expect(
              ...
              equals(scenario.domainState.drawnCard?.value),
          ...
          if (scenario.expectations['game_completed'] == true) {
            expect(
              ...
              reason:
                  '${scenario.name}: Completed game must have finished status',
            );
            expect(
              reconstructed.finishedAt,
              isNotNull,
              reason:
                  '${scenario.name}: Completed game must have finish timestamp',
            );
            expect(
              model.winnerId,
              equals('winner'),
          ...
          if (scenario.domainState.players.isNotEmpty) {
            ...
            expect(
              firstPlayer.id,
              equals(originalFirst.id),
              reason: '${scenario.name}: Player ID must persist',
            );
            expect(
              firstPlayer.name,
              equals(originalFirst.name),
              reason: '${scenario.name}: Player name must persist',
            );
            expect(
              firstPlayer.isHost,
              equals(originalFirst.isHost),
              reason: '${scenario.name}: Host status must persist',
            );
            expect(
              firstPlayer.actionCards.length,
              equals(originalFirst.actionCards.length),
    ...
    test(
      'should maintain data contract stability for API and storage compatibility',
      () {
        ...
        final serializationScenarios = [
          ...
          (
            ...
            expectations: {'minimal_fields': true, 'no_optional_fields': true},
          ),
          ...
          (
            ...
            model: GameStateModel(
              ...
              gameData: {
                ...
                'deck': List.generate(
                  30,
                  (i) => {'value': i % 13 - 2, 'isRevealed': false},
                ),
                'discardPile': List.generate(
                  15,
                  (i) => {'value': i % 13 - 2, 'isRevealed': true},
                ),
                'actionDeck': List.generate(
                  5,
                  (i) => {
        ...
        for (final scenario in serializationScenarios) {
          ...
          for (final field in requiredFields) {
            expect(
              ...
              isTrue,
              reason:
                  '${scenario.name}: Required field "$field" must be present',
            );
            expect(
              json[field],
              isNotNull,
              reason:
                  '${scenario.name}: Required field "$field" cannot be null',
            );
          }
          ...
          if (scenario.model.winnerId == null) {
            ...
            expect(
              json['winner_id'],
              equals(scenario.model.winnerId),
          ...
          if (scenario.model.endedAt == null) {
            expect(
              ...
              isNull,
              reason:
                  '${scenario.name}: Null end time should serialize as null',
            );
          } else {
            expect(
              json['ended_at'],
          ...
          expect(
            supabaseJson.containsKey('current_player_id'),
            isTrue,
            reason: '${scenario.name}: Supabase format must use snake_case',
          );
          expect(
            supabaseJson.containsKey('turn_number'),
            isTrue,
            reason: '${scenario.name}: Supabase format must use snake_case',
          );
          expect(
          ...
          expect(
            ...
            reason:
                '${scenario.name}: Timestamps must be ISO8601 with milliseconds',
          );
          expect(
          ...
          if (scenario.expectations['complex_nested_data'] == true) {
            ...
            expect(
              ...
              hasLength(2),
              reason:
                  '${scenario.name}: Complex state should preserve player data',
            );
            expect(
              gameData['deck'],
              hasLength(30),
              reason:
                  '${scenario.name}: Complex state should preserve deck size',
            );
            expect(
              gameData['drawnCard'],
              isNotNull,
              reason:
                  '${scenario.name}: Complex state should preserve drawn card',
            );
          }
          ...
          if (scenario.expectations['historical_completeness'] == true) {
            expect(
              ...
              equals('winner-id'),
              reason:
                  '${scenario.name}: Historical record must preserve winner',
            );
            expect(
              json['ended_at'],
              isNotNull,
              reason:
                  '${scenario.name}: Historical record must have end timestamp',
            );
            final gameData = json['game_data'] as Map<String, dynamic>;
            expect(
              gameData['finishedAt'],
              isNotNull,
              reason:
                  '${scenario.name}: Historical record must preserve finish time in game data',
            );
          }
        }
      },

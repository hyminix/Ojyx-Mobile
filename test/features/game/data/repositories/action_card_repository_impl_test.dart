...
void main() {
  ...
  group('ActionCardRepositoryImpl', () {
    ...
    test('should add action card to player through datasource', () {
      // Arrange
      final playerId = 'player1';
      when(() => mockDataSource.addActionCardToPlayer(playerId, testCard))
          .thenAnswer((_) {});
      ...
      // Assert
      verify(() => mockDataSource.addActionCardToPlayer(playerId, testCard)).called(1);
    });
    
    test('should remove action card from player through datasource', () {
      // Arrange
      final playerId = 'player1';
      when(() => mockDataSource.removeActionCardFromPlayer(playerId, testCard))
          .thenAnswer((_) {});
      ...
      // Assert
      verify(() => mockDataSource.removeActionCardFromPlayer(playerId, testCard)).called(1);
    });
    ...
    test('should discard action card through datasource', () {
      // Arrange
      when(() => mockDataSource.discardActionCard(testCard))
          .thenAnswer((_) {});
    ...
    test('should propagate exception when adding card to player with full hand', () {
      // Arrange
      final playerId = 'player1';
      when(() => mockDataSource.addActionCardToPlayer(playerId, testCard))
    ...
    test('should propagate exception when removing card player does not have', () {
      // Arrange
      final playerId = 'player1';
      when(() => mockDataSource.removeActionCardFromPlayer(playerId, testCard))

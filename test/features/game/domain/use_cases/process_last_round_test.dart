...
void main() {
  ...
  group('ProcessLastRound UseCase', () {
    test('should enable strategic endgame information asymmetry for competitive advantage', () async {
      ...
      final strategicScenarios = [
          ...
          'players': [
            GamePlayer(
              ...
              grid: PlayerGrid.empty()
                  .placeCard(const Card(value: -2, isRevealed: true), 0, 0)  // Known low card
                  .placeCard(const Card(value: 12, isRevealed: false), 0, 1) // Hidden high card
                  .placeCard(const Card(value: 0, isRevealed: false), 1, 0)  // Hidden neutral
                  .placeCard(const Card(value: -1, isRevealed: false), 2, 2), // Hidden bonus
              isHost: true,
            ),
            GamePlayer(
              id: 'challenger-456',
              name: 'Challenger',
              grid: PlayerGrid.empty()
                  .placeCard(const Card(value: 3, isRevealed: false), 0, 0)  // Will be revealed
                  .placeCard(const Card(value: 7, isRevealed: false), 0, 1)  // Will be revealed
                  .placeCard(const Card(value: -2, isRevealed: true), 1, 0)  // Already visible
                  .placeCard(const Card(value: 5, isRevealed: false), 2, 3), // Will be revealed
            ),
            GamePlayer(
              id: 'competitor-789',
              name: 'Competitor',
              grid: PlayerGrid.empty()
                  .placeCard(const Card(value: 10, isRevealed: false), 0, 0) // Will be revealed
                  .placeCard(const Card(value: 2, isRevealed: false), 1, 1)  // Will be revealed
                  .placeCard(const Card(value: 8, isRevealed: false), 2, 2)  // Will be revealed
                  .placeCard(const Card(value: 1, isRevealed: false), 2, 3), // Will be revealed
            ),
          ],
          'expectedBehavior': (GameState result) {
            // Initiator maintains strategic advantage
            final initiator = result.players.firstWhere((p) => p.id == 'strategic-master-123');
            expect(initiator.grid.cards[0][0]?.isRevealed, true, reason: 'Previously revealed stays visible');
            expect(initiator.grid.cards[0][1]?.isRevealed, false, reason: 'Hidden high card stays secret');
            expect(initiator.grid.cards[1][0]?.isRevealed, false, reason: 'Hidden neutral stays secret');
            expect(initiator.grid.cards[2][2]?.isRevealed, false, reason: 'Hidden bonus stays secret');
            
            // Challenger forced to reveal all
            final challenger = result.players.firstWhere((p) => p.id == 'challenger-456');
            expect(challenger.grid.cards[0][0]?.isRevealed, true, reason: 'Hidden cards now visible');
            expect(challenger.grid.cards[0][1]?.isRevealed, true, reason: 'All cards exposed');
            expect(challenger.grid.cards[1][0]?.isRevealed, true, reason: 'Already visible stays visible');
            expect(challenger.grid.cards[2][3]?.isRevealed, true, reason: 'Strategic position revealed');
            
            // Competitor fully exposed
            final competitor = result.players.firstWhere((p) => p.id == 'competitor-789');
            for (int row = 0; row < 3; row++) {
              for (int col = 0; col < 4; col++) {
                final card = competitor.grid.cards[row][col];
                if (card != null) {
                  expect(card.isRevealed, true, reason: 'All competitor cards must be visible');
                }
              }
            }
          },
        ...
        // Scenario 2: Large tournament with strategic information control
        {
          ...
          'players': List.generate(8, (i) => GamePlayer(
            ...
            grid: PlayerGrid.empty()
                ...
                .placeCard(Card(value: (i * 2) % 13 - 2, isRevealed: false), 0, 1)
          ...
          'expectedBehavior': (GameState result) {
            ...
            final leader = result.players.firstWhere((p) => p.id == 'tournament-leader-001');
            int hiddenCount = 0;
            int revealedCount = 0;
            ...
            for (final row in leader.grid.cards) {
              for (final card in row) {
                if (card != null) {
                  if (card.isRevealed) revealedCount++;
                  else hiddenCount++;
                }
              }
            }
            
            expect(hiddenCount > 0, true, reason: 'Leader must maintain some hidden information');
                  ...
                  for (final card in row) {
                    if (card != null) {
                      expect(card.isRevealed, true, 
                        reason: 'Player ${player.name} must have all cards visible in endgame');
                    }
                  }
                }
              }
            ...
            for (final player in result.players) {
              for (final row in player.grid.cards) {
                for (final card in row) {
                  if (card != null) {
                    expect(card.isRevealed, true, 
                      reason: 'Without clear initiator, all cards visible for fair endgame');
                  }
                }
              }
            }
      ...
      for (final scenario in strategicScenarios) {
        ...
        expect(result.isRight(), true, 
          reason: 'Scenario "${scenario['description']}" should process successfully');

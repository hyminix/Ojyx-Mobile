...
void main() {
  ...
  group('DistributeCards UseCase', () {
    test('should ensure each player receives exactly their allocated cards', () async {
      final players = [
        GamePlayer(
          id: 'player1',
          name: 'Alice',
      ...
      result.fold((failure) => fail('Should not fail'), (gameState) {
        ...
        final player1 = gameState.players.firstWhere((p) => p.id == 'player1');
        final player2 = gameState.players.firstWhere((p) => p.id == 'player2');
        final player3 = gameState.players.firstWhere((p) => p.id == 'player3');

        // Each player must have exactly 12 cards (3x4 grid filled)
        expect(_countPlayerCards(player1), kCardsPerPlayer, 
               reason: 'Player1 (Alice) should have received exactly 12 cards');
        expect(_countPlayerCards(player2), kCardsPerPlayer,
               reason: 'Player2 (Bob) should have received exactly 12 cards');
        expect(_countPlayerCards(player3), kCardsPerPlayer,
               reason: 'Player3 (Charlie) should have received exactly 12 cards');

        // Verify cards are face down as dealt
        expect(_areAllCardsFaceDown(player1), true,
               reason: 'All of Alice\'s cards should be face down initially');
        expect(_areAllCardsFaceDown(player2), true,
               reason: 'All of Bob\'s cards should be face down initially');
        expect(_areAllCardsFaceDown(player3), true,
        ...
        final expectedRemainingInDeck = initialState.deck.length - totalDistributed - 1; // -1 for discard
        expect(gameState.deck.length, expectedRemainingInDeck,
               reason: 'Deck should be reduced by exactly the number of cards distributed plus discard');
      });
    });

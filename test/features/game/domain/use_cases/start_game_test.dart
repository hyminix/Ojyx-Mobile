...
void main() {
  ...
  group('StartGame UseCase', () {
    test('should create fully playable competitive game environment with strategic card distribution', () async {
      ...
      final gameScenarios = [
        // Scenario 1: Standard 2-player competitive match
        {
          ...
          'expectedBehavior': (GameState result) {
            ...
            expect(result.currentPlayerIndex! >= 0 && 
                   result.currentPlayerIndex! < result.players.length, true,
            ...
            for (final player in result.players) {
              ...
              expect(cardCount, kCardsPerPlayer, 
                reason: 'Player ${player.name} must have full deck distributed');
              expect(revealedCount, kInitialRevealedCards,
                reason: 'Player ${player.name} must have strategic initial visibility');
            }
            
            // Verify shared game elements
            expect(result.discardPile.length, 1,
              reason: 'Must have initial discard to start gameplay');
            expect(result.discardPile.first.isRevealed, true,
              reason: 'Initial discard must be visible to all');
            expect(result.deck.isNotEmpty, true,
        ...
        // Scenario 2: Large tournament with maximum players
        {
          ...
          'expectedBehavior': (GameState result) {
            ...
            final totalCardsDistributed = result.players.fold<int>(
              0,
              (sum, player) {
                int playerCards = 0;
                for (final row in player.grid.cards) {
            ...
            expect(totalCardsDistributed, kMaxPlayers * kCardsPerPlayer,
            ...
            for (final player in result.players) {
              int reveals = 0;
              for (final row in player.grid.cards) {
                reveals += row.where((card) => card?.isRevealed == true).length;
              }
              expect(reveals, kInitialRevealedCards,
        ...
        // Scenario 3: Minimum viable game
        {
          ...
          'players': [
            GamePlayer(id: 'player1', name: 'Player 1', grid: PlayerGrid.empty(), isHost: true),
          ...
          'expectedBehavior': (GameState result) {
            ...
            for (final player in result.players) {
              ...
              expect(revealedCards, kInitialRevealedCards,
                reason: 'Default positions must reveal correct number of cards');
            }
          },
        },
      ];
      ...
      for (final scenario in gameScenarios) {
        ...
        final result = await startGame(
          StartGameParams(
            ...
            playerInitialPositions: scenario['initialPositions'] as Map<String, List<(int, int)>>?,
          ),
        );
        
        expect(result.isRight(), true,
          reason: 'Scenario "${scenario['description']}" should start successfully');
        ...
        result.fold(
          ...
          (gameState) {
            ...
            expect(gameState.status, GameStatus.playing,
              reason: 'Game must be in playing state for "${scenario['description']}"');
      ...
      final validationScenarios = [
        {
          ...
          'setup': () => GameState.initial(
            ...
            players: [GamePlayer(id: 'solo', name: 'Solo Player', grid: PlayerGrid.empty(), isHost: true)],
          ),
          'positions': {'solo': [(0, 0), (1, 1)]},
        },
        {
          ...
          'setup': () => GameState.initial(
            ...
            players: [
              GamePlayer(id: 'p1', name: 'P1', grid: PlayerGrid.empty(), isHost: true),
              GamePlayer(id: 'p2', name: 'P2', grid: PlayerGrid.empty()),
            ],
          ).copyWith(status: GameStatus.playing),
          'positions': {'p1': [(0, 0), (1, 1)], 'p2': [(0, 1), (1, 2)]},
        },
      ];
      ...
      for (final validation in validationScenarios) {
        ...
        final result = await startGame(
          StartGameParams(
            gameState: gameState,
            playerInitialPositions: validation['positions'] as Map<String, List<(int, int)>>,
          ),
        );

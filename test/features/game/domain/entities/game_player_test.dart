...
void main() {
  group('GamePlayer Action Card Management', () {
    test(
      'should expand action card options when receiving new cards during gameplay',
      () {
        ...
        expect(
          player.actionCards,
          isEmpty,
          reason: 'Original player should remain unchanged',
        );
        expect(
          updatedPlayer.actionCards.length,
          1,
          reason: 'Player should gain strategic option',
        );
        expect(
          updatedPlayer.actionCards.first,
          equals(actionCard),
          reason: 'Exact card should be available',
        );
      },
    );

    test(
      'should enforce hand limit to maintain game balance when action cards are abundant',
      () {
        ...
        expect(
          player.actionCards.length,
          3,
          reason: 'Player should reach maximum hand size',
        );
    ...
    test('should allow strategic card usage by removing cards from hand', () {
      ...
      expect(
        player.actionCards.length,
        2,
        reason: 'Player should have multiple strategic options',
      );
      ...
      expect(
        updatedPlayer.actionCards.length,
        1,
        reason: 'Player should have fewer options after using card',
      );
      expect(
        updatedPlayer.actionCards.first.id,
        'reverse-1',
        reason: 'Remaining card should be available',
      );
    });

    test(
      'should calculate meaningful score for competitive ranking during end game',
      () {
        ...
        expect(
          ...
          expectedScore,
          reason:
              'Score should reflect revealed card values for competitive ranking',
        );
      },
    );
  });

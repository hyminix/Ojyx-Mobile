    ...
    test('should manage deck state properties correctly for gameplay', () {
      ...
      final testCases = [
        ...
        (
          ...
          1,
          true,
          'empty draw pile, full discard',
        ),
        (
          [testCards[0]],
          [testCards[1], testCards[2]],
          1,
          10,
          false,
          'mixed state with cards in both piles',
        ),
      ];
      ...
      for (final (
        ...
        expect(
          deckState.remainingDrawCount,
          expectedCount,
          reason: 'Draw count should be correct for $description',
        );
        expect(
          deckState.isDrawPileEmpty,
          expectedEmpty,
      ...
      final operationTestCases = [
        ...
        () {
          ...
          expect(
            drawnCard,
            isNotNull,
            reason: 'Should draw card from non-empty pile',
          );
          expect(
            drawnCard!.value,
            equals(5),
            reason: 'Should draw first card from pile',
          );
          expect(
            newState.drawPile.length,
            equals(3),
            reason: 'Draw pile should decrease by 1',
          );
          expect(
            newState.discardPile.length,
            equals(0),
            reason: 'Discard pile should remain unchanged',
          );
        },
        ...
        () {
          ...
          expect(
            drawnCard,
            isNull,
            reason: 'Should return null when drawing from empty pile',
          );
          expect(
            newState.drawPile,
            isEmpty,
            reason: 'Empty draw pile should remain empty',
          );
          expect(
            newState.discardPile,
            isEmpty,
            reason: 'Empty discard pile should remain empty',
          );
        },
        ...
        () {
          ...
          expect(
            newState.discardPile.length,
            equals(2),
            reason: 'Discard pile should grow by 1',
          );
          expect(
            newState.discardPile.last,
            equals(cardToDiscard),
            reason: 'Discarded card should be on top',
          );
          expect(
            newState.drawPile.length,
            equals(2),
            reason: 'Draw pile should remain unchanged',
          );
        },
      ];
    ...
    test('should handle deck reshuffling behavior for continuous gameplay', () {
      ...
      final reshuffleTestCases = [
        ...
        (
          ...
          0,
          0,
          'no reshuffle with empty discard',
        ),
        (
          [testCards[0]],
          [testCards[1], testCards[2]],
          ...
          1,
          1,
          'reshuffle with existing draw pile',
        ),
      ];
      ...
      for (final (
          ...
          in reshuffleTestCases) {
        final deckState = DeckState(
          drawPile: initialDraw,
        ...
        if (shouldReshuffle) {
          ...
          final totalCards =
              newState.drawPile.length + newState.discardPile.length;
          final expectedTotal =
              (initialDiscard.length - 1) +
              1; // Cards moved from discard minus top card + top card kept
          expect(
            totalCards,
            expectedTotal,
            reason:
                'Total card count should match reshuffled cards for $description',
          );
        } else {
          expect(
            newState,

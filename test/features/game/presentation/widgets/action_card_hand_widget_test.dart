...
void main() {
  group('ActionCardHand Strategic Hand Management Behavior', () {
    ...
    testWidgets(
      'should enforce immediate action card priority for game flow integrity',
      (tester) async {
        ...
        expect(
          immediateCardWidget.isHighlighted,
          isTrue,
          reason: 'Immediate cards create mandatory strategic decisions',
        );
        expect(
          immediateCardWidget.card.timing,
          ActionTiming.immediate,
          reason:
              'Turn reversal must execute immediately affecting all players',
        );
      },
    );

    testWidgets(
      'should enable strategic card activation only during player turn',
      (tester) async {
        ...
        final activationScenarios = [
          (
            ...
            'current player can execute strategic actions',
          ),
          (
            isCurrentPlayer: false,
            expectActivation: false,
            'opponents cannot hijack turn order',
          ),
        ];

        for (final (:isCurrentPlayer, :expectActivation, scenario)
            in activationScenarios) {
    ...
    testWidgets(
      'should communicate hand capacity constraints for strategic planning',
      (tester) async {
        // Test behavior: capacity limits force strategic card management decisions
        final capacityScenarios = [
          (
            actionCards: strategicCards,
            display: 'Cartes Actions (3/3)',
            'full capacity forces discard decisions',
          ),
          (
            actionCards: strategicCards.take(2).toList(),
            display: 'Cartes Actions (2/3)',
            'available space allows new acquisitions',
          ),
          (
            actionCards: <ActionCard>[],
            display: 'Aucune carte action',
            'empty hand limits strategic options',
          ),
        ];
        ...
        for (final (:actionCards, :display, scenario) in capacityScenarios) {
          ...
          if (actionCards.isEmpty) {
            ...
            expect(
              find.byIcon(Icons.do_not_disturb),
              findsOneWidget,
            ...
            expect(
              ...
              findsOneWidget,
              reason:
                  'Capacity awareness enables strategic planning: $scenario',
            );
          }
        }
      },
    ...
    testWidgets(
      'should categorize cards by strategic timing for tactical awareness',
      (tester) async {
        ...
        final cardWidgets = tester
            .widgetList<ActionCardWidget>(find.byType(ActionCardWidget))
            .toList();

        final timingCategories = {
          ActionTiming.optional:
              'flexible timing for optimal strategic deployment',
        ...
        for (final widget in cardWidgets) {
          ...
          expect(
            timingCategories.containsKey(timing),
            isTrue,
    ...
    testWidgets(
      'should support strategic card discarding for hand optimization',
      (tester) async {
        ...
        expect(
          ...
          isNotNull,
          reason:
              'Long press enables strategic hand management through selective discarding',
        );
      },
    );

    testWidgets(
      'should prevent concurrent card activations maintaining game integrity',
      (tester) async {
        ...
        await tester.tap(
          find.byType(ActionCardWidget).first,
        ); // Should be blocked
        await tester.pump();
        ...
        expect(
          ...
          1,
          reason:
              'Animation lock prevents duplicate strategic actions corrupting game state',
        );
    ...
    testWidgets('should adapt hand visibility based on strategic context', (
      tester,
    ) async {
      ...
      final visibilityScenarios = [
        (
          isCurrentPlayer: true,
          expectFullDetails: true,
          'current player needs full strategic information',
        ),
        (
          isCurrentPlayer: false,
          expectFullDetails: false,
          'opponents see limited information for competitive balance',
        ),
      ];

      for (final (:isCurrentPlayer, :expectFullDetails, scenario)
          in visibilityScenarios) {
        await tester.pumpWidget(
          createTestWidget(
            player: testPlayer,
            isCurrentPlayer: isCurrentPlayer,

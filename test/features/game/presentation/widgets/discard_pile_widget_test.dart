...
void main() {
  group('DiscardPile Strategic Discard Management Behavior', () {
    ...
    testWidgets(
      'should reveal strategic information through visible discard values',
      (tester) async {
        // Test behavior: discard pile provides competitive intelligence
        final strategicCards = [
          (
            card: const game.Card(value: -2, isRevealed: true),
            insight: 'bonus card discarded reveals risk tolerance',
          ),
          (
            card: const game.Card(value: 12, isRevealed: true),
            insight: 'penalty discard shows defensive play',
          ),
          (
            card: const game.Card(value: 5, isRevealed: true),
            insight: 'neutral discard indicates calculated strategy',
          ),
        ...
        for (final (:card, :insight) in strategicCards) {
          ...
          if (card != null) {
            ...
            expect(
              find.byIcon(Icons.layers_clear),
              findsOneWidget,
    ...
    testWidgets(
      'should enable strategic discard actions based on game context',
      (tester) async {
        ...
        for (final (:canDiscard, :onTap, :expectInteraction, :scenario)
            in discardScenarios) {
    ...
    testWidgets(
      'should track discard patterns for strategic opponent analysis',
      (tester) async {
    ...
    testWidgets(
      'should communicate discard accessibility for inclusive gameplay',
      (tester) async {
        ...
        expect(
          ...
          findsOneWidget,
          reason:
              'Screen readers announce discard state for visually impaired players',
        );
      },
    );

    testWidgets(
      'should enable strategic card disposal through drag-drop mechanics',
      (tester) async {
        ...
        for (final (:canDiscard, :cardToDispose, :expectSuccess, :strategy)
            in disposalScenarios) {

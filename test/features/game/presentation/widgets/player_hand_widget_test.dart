...
void main() {
  group('PlayerHandWidget Strategic Card Management Behavior', () {
    testWidgets(
      'should enable strategic card decision-making for current player only',
      (WidgetTester tester) async {
        ...
        for (final (drawnCard, isCurrentPlayer, expectedBehavior)
            in scenarios) {
          ...
          final hasStrategicInterface = find
              .text('Carte en main')
              .evaluate()
              .isNotEmpty;
    ...
    testWidgets(
      'should provide strategic discard decision based on game state constraints',
      (WidgetTester tester) async {
        ...
        for (final (value, canDiscard, expectOption, scenario)
            in gameScenarios) {
          ...
          expect(hasDiscardOption, expectOption, reason: 'Scenario: $scenario');

          // Verify strategic guidance is provided
          if (expectOption) {
            expect(
              find.text('Glissez sur\nune carte'),
              findsOneWidget,
    ...
    testWidgets(
      'should visualize strategic hand position to influence player decisions',
      (WidgetTester tester) async {
    ...
    testWidgets('should handle edge cases gracefully maintaining game integrity', (
      ...
      final edgeCases = [
        ...
        (
          const game.Card(value: 0, isRevealed: false),
          true,
          true,
          null, // Null callback
      ...
      for (final (card, isCurrent, canDiscard, onDiscard, scenario)
          in edgeCases) {
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(

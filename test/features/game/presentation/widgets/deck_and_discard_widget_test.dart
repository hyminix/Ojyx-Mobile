...
void main() {
  group('DeckAndDiscard Strategic Draw Decision Behavior', () {
    ...
    testWidgets('should enable strategic draw decisions between hidden risk and known values', (tester) async {
    ...
    testWidgets('should reveal discard value enabling strategic risk assessment', (tester) async {
      ...
      for (final (value: cardValue, :decision) in strategicScenarios) {
        ...
        expect(discardWidget.card?.value, equals(cardValue),
            reason: 'Visible value enables strategic assessment: $decision');
      }
    });

    testWidgets('should force deck-only strategy when discard empty', (tester) async {
    ...
    testWidgets('should execute strategic draw choices based on game context and turn rules', (tester) async {
      ...
      for (final (:source, :setupDiscard, :canDraw, :expectAction, :strategy) in drawScenarios) {
          ...
          MaterialApp(
            home: Scaffold(
              body: DeckAndDiscardWidget(
                gameState: gameState,
                canDraw: canDraw,
                onDrawFromDeck: source == 'deck' ? () => actionExecuted = true : null,
                onDrawFromDiscard: source == 'discard' ? () => actionExecuted = true : null,
              ),
            ),
          ),
        );
        ...
        expect(actionExecuted, expectAction,
            reason: 'Strategic scenario: $strategy');
      }
    });

    testWidgets('should communicate strategic draw opportunities through visual indicators', (tester) async {
      ...
      for (final (:discardPile, :canDraw, :expectedIndicators, :scenario) in indicatorScenarios) {
        final gameState = mockGameState.copyWith(discardPile: discardPile);

        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: DeckAndDiscardWidget(gameState: gameState, canDraw: canDraw),
            ),
          ),
        );

        expect(find.byIcon(Icons.touch_app), findsNWidgets(expectedIndicators),
            reason: 'Visual guidance for: $scenario');
      }
    });

    testWidgets('should track deck depletion for endgame strategic planning', (tester) async {
      // Test behavior: deck count awareness influences draw strategies
      final depletionScenarios = [
        (count: 52, display: '52 cartes', strategy: 'abundant deck allows aggressive drawing'),
        (count: 10, display: '10 cartes', strategy: 'low deck signals conservative play'),
        (count: 2, display: '2 cartes', strategy: 'critical depletion forces adaptation'),
      ];

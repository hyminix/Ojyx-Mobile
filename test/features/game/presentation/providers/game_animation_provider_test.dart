...
void main() {
  group('Game Animation Visual Feedback System', () {
    ...
    test(
      'should manage visual direction change feedback for competitive turn order awareness',
      () {
        ...
        expect(
          state.showingDirectionChange,
          false,
          reason: 'Initial state should have no active visual feedback',
        );
        expect(
          state.direction,
          PlayDirection.forward,
          reason: 'Default direction should be forward',
        );
        ...
        expect(
          state.showingDirectionChange,
          true,
          reason: 'Visual feedback should activate for strategic awareness',
        );
        expect(
          state.direction,
          PlayDirection.backward,
          reason: 'Direction change should be visually communicated',
        );
        ...
        expect(
          state.showingDirectionChange,
          false,
          reason: 'Visual feedback should complete gracefully',
        );
        expect(
          state.direction,
          PlayDirection.backward,
          reason: 'Direction context should be preserved',
        );
      },
    );

    test(
      'should enforce single-animation policy for clear visual communication',
      () {
        ...
        expect(
          state.direction,
          PlayDirection.forward,
          reason: 'First animation should establish direction',
        );
        ...
        expect(
          state.direction,
          PlayDirection.forward,
          reason: 'Original animation should have priority',
        );
        expect(
          state.showingDirectionChange,
          true,
          reason: 'Animation conflict should not interrupt active feedback',
        );
        ...
        expect(
          state.direction,
          PlayDirection.backward,
          reason: 'Sequential animation should work after completion',
        );
      },
    );
  });

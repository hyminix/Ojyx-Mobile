...
void main() {
  group('CardSelectionProvider Strategic Game Behavior', () {
    ...
    test(
      'should enable complete strategic card action workflows for competitive gameplay',
      () {
        ...
        expect(
          state.isSelecting,
          isTrue,
          reason: 'Strategic action mode should be active',
        );
        expect(
          state.selectionType,
          CardSelectionType.teleport,
          reason: 'Teleport strategy should be selected',
        );
        ...
        expect(
          teleportData,
          isNotNull,
          reason: 'Strategic teleport should produce action data',
        );
        expect(
          teleportData!['position1'],
          isNotNull,
          reason: 'Source position should be captured',
        );
        expect(
          teleportData['position2'],
          isNotNull,
          reason: 'Target position should be captured',
        );
        ...
        expect(
          ...
          3,
          reason: 'Intelligence gathering should track all targets',
        );
        ...
        expect(
          peekData!['positions'],
          isA<List>(),
          reason: 'Intelligence operation should return all scouted positions',
        );
        ...
        expect(
          ...
          'rival-player',
          reason: 'Competitive target should be locked in',
        );
        expect(
          state.isSelectionComplete,
          isTrue,
          reason: 'Dual-phase action should be ready for execution',
        );

        final stealData = notifier.completeSelection();
        expect(
          stealData!['opponentId'],
          'rival-player',
          reason: 'Target opponent should be preserved',
        );
        expect(
          stealData['position'],
          isNotNull,
          reason: 'Strategic card position should be captured',
        );
      },
    );

    test(
      'should support strategic action mode switching for tactical flexibility',
      () {
        ...
        final actionScenarios = [
          ...
          (
            CardSelectionType.selectOpponent,
            () => notifier.startOpponentSelection(),
            'competitive targeting',
          ),
        ];

        for (final (expectedType, actionStarter, description)
            in actionScenarios) {
          actionStarter();
          ...
          expect(
            state.isSelecting,
            isTrue,
            reason: 'Strategic $description mode should be active',
          );
          expect(
            state.selectionType,
            expectedType,
            reason: '$description action should be properly configured',
          );
        ...
        expect(
          finalState.selectionType,
          CardSelectionType.swap,
          reason: 'New strategy should override previous',
        );
        expect(
          finalState.firstSelection,
          isNull,
          reason: 'Previous selections should be cleared for clean transition',
        );
        expect(
          finalState.secondSelection,
          isNull,
          reason: 'Strategic state should be reset for new action',
        );
      },
    );

    test(
      'should enforce intelligent capacity management and strategic selection patterns',
      () {
        ...
        expect(
          state.selections.length,
          2,
          reason: 'Intelligence capacity should be enforced',
        );
        expect(
          state.selections.any((s) => s.row == 0 && s.col == 0),
          false,
          reason: 'Oldest intelligence should be rotated out',
        );
        expect(
          state.selections.any((s) => s.row == 0 && s.col == 2),
          true,
          reason: 'New intelligence should be retained',
        );
        ...
        expect(
          state.selections.length,
          1,
          reason: 'Strategic deselection should reduce capacity usage',
        );
        ...
        expect(
          state.isSelectionComplete,
          true,
          reason: 'Dual-position strategy should be complete',
        );
        ...
        state = container.read(cardSelectionProvider);
        expect(
          state.firstSelection?.row,
          2,
          reason: 'Strategic repositioning should update first target',
        );
        expect(
          state.secondSelection,
          isNull,
          reason:
              'Strategic cycle should clear second position for fresh selection',
        );
        ...
        state = container.read(cardSelectionProvider);
        expect(
          state.hasFirstSelection,
          false,
          reason: 'Strategic cancellation should clear position',
        );
        ...
        expect(
          state.hasFirstSelection,
          false,
          reason: 'System should protect against invalid selection contexts',
        );
      },
    );

    test(
      'should provide precise position tracking for strategic decision making',
      () {
        ...
        for (final ((row, col), expectedSelected, description)
            in positionChecks) {
          final isSelected = notifier.isPositionSelected(row, col);
          expect(
            isSelected,
            expectedSelected,
            reason: 'Position tracking should accurately identify $description',
          );
        }

        // Test specific role-based position identification
        expect(
          notifier.isFirstSelection(0, 1),
          true,
          reason: 'Primary strategic position should be correctly identified',
        );
        expect(
          notifier.isSecondSelection(1, 2),
          true,
          reason: 'Secondary strategic position should be correctly identified',
        );
        expect(
          notifier.isFirstSelection(1, 2),
          false,
          reason: 'Position role distinction should be maintained',
        );
      },
    );

    test(
      'should enable strategic action cancellation for tactical flexibility',
      () {
        ...
        expect(
          state.isSelecting,
          false,
          reason: 'Strategic mode should be deactivated',
        );
        expect(
          state.selectionType,
          isNull,
          reason: 'Action type should be cleared',
        );
        expect(
          state.firstSelection,
          isNull,
          reason: 'All strategic positions should be cleared',
        );
        expect(
          state.secondSelection,
          isNull,
          reason: 'Complete selection state should be reset',
        );
        ...
        expect(
          () => notifier.cancelSelection(),
          returnsNormally,
          reason: 'Repeated cancellation should be safe',
        );
      },
    );

    test(
      'should deliver complete strategic data structures for game action execution',
      () {
        ...
        final strategicScenarios = [
          ...
          () {
            ...
            expect(
              data!['position1']['row'],
              0,
              reason: 'Source position should be accurately captured',
            );
            expect(
              data['position2']['col'],
              3,
              reason: 'Target position should be precisely recorded',
            );
          },
          ...
          () {
            ...
            expect(
              data!['positions'],
              isA<List>(),
              reason: 'Intelligence data should be in list format',
            );
            expect(
              data['positions'].length,
              3,
              reason: 'All scouted positions should be included',
            );
          },
        ...
        expect(
          ...
          isNull,
          reason:
              'Incomplete strategic actions should not produce execution data',
        );
        ...
        expect(
          postExecutionState.isSelecting,
          false,
          reason: 'Strategic mode should reset after action execution',
        );
      },
    );
  });

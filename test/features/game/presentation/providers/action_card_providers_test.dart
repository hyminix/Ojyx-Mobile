...
void main() {
  ...
  group('ActionCard Provider Behavior', () {
    test('should update game state when action card is used successfully', () async {
      ...
      when(() => mockUseCase(any())).thenAnswer(
        (_) async => Right({'success': true, 'updated_state': true}),
      );
    ...
    test('should handle action card usage failure', () async {
      ...
      // Setup mock failure
      when(() => mockUseCase(any())).thenAnswer(
        (_) async => Left(Failure.gameLogic(message: 'Card not found', code: 'CARD_NOT_FOUND')),
      );
